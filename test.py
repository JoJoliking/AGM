...
from utils.loss import mot_loss

def test_loader_cal_loss(model, dataloader, device, Test_Loss):
    ...
    cal_mot_loss = Test_Loss
    for i ,(imgs, targets,paths,_) in pbar:
        imgs = imgs.to(device, non_blocking=True).float() / 255.0
        pred = model(imgs.to(device))
        loss+=cal_mot_loss(pred[1],targets.to(device),model)[1][3]
    print('Value Dataset Loss is {}'.format(loss/nb))
    return loss/nb
...
def test(data,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            # Run model
            t = time_synchronized()
            # inf_out for pre results; train out for train results
            ...
            merge为是否合并框
            """
            output = non_max_suppression(inf_out[..., :6], conf_thres=conf_thres, iou_thres=iou_thres)
