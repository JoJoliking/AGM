...
def labels_to_class_weights(labels, nc=80):
    ...
    'For Only one class. Can be define in 0.'
    labels[:, 0] = 0
...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, merge=False, classes=None, agnostic=False):
    ...
    prediction:
    (x, y, w, h, conf, 80 class scores, 512 re-id)
    Returns:
    ...
    t = time.time()
    output = [torch.zeros(0, 6+128)] * prediction.shape[0]
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        i = cluster_diounms(boxes, scores, iou_thres)  # CIoU
        ################################ NMS ################################
